...
from conftest import StubClient, analytics_result, video

UTC = timezone.utc


def test_first_run_records_existing_videos_without_notifying(
    write_state, run_cycle
):
    write_state()
    result = run_cycle(StubClient(videos=[video("v1"), video("v2")]))

    assert result.account()["known_video_ids"] == ["v1", "v2"]
    assert result.notifier.calls == []
    assert result.pending == []


def test_new_post_is_notified_and_scheduled(write_state, run_cycle):
    write_state(accounts={"@acct": {"known_video_ids": ["v1"]}})
    result = run_cycle(StubClient(videos=[video("v2", hours_ago=1),
                                          video("v1")]))
...
def test_first_post_from_a_previously_empty_account_is_notified(
    ...
    write_state(accounts={"@acct": {"known_video_ids": []}})
    result = run_cycle(StubClient(videos=[video("v1", hours_ago=1)]))

    assert result.notifier.only("new_post")["video_id"] == "v1"


def test_known_videos_are_not_renotified(write_state, run_cycle):
    write_state(accounts={"@acct": {"known_video_ids": ["v1", "v2"]}})
...
def test_slack_failure_does_not_lose_the_detection(write_state, run_cycle):
...
def test_one_account_failing_does_not_stop_the_others(write_state, run_cycle):
    from tiktok_client import TikTokClientError

    write_state(
        accounts={
            "@ok": {"known_video_ids": ["v1"]},
            "@broken": {"known_video_ids": ["v9"]},
        }
    )
    client = StubClient(
        videos={
    ...
    assert "v2" in result.state["accounts"]["@ok"]["known_video_ids"]
...
class TestPostTimeAnchoring:
    ...
    def test_lateness_allowance_is_configurable(self, write_state, run_cycle):
        write_state(accounts={"@acct": {"known_video_ids": ["v1"]}})
        result = run_cycle(
            StubClient(videos=[video("v2", hours_ago=31), video("v1")],
                       analytics=analytics_result()),
